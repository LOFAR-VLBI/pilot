class: Workflow
cwlVersion: v1.2
id: delay-calibration-lba
label: LBA VLBI delay calibration
doc: |
    The delay calibration pipeline does the following:
    * applies LiLF solutions to target data and flags
      A-team sources,
    * corrects direction-dependent effects for the Dutch stations and
      optionally subtracts the LiLF model outside a user-specifiable region.
...
inputs:
    ...
    - id: lilf_dir
      type: Directory?
      doc: |
        Path to the directory where lilf was run, where the calibration solutions, facet regions and final model exist

    - id: box_size
      type: float?
      default: 2.5
      doc: |
        [Required if subtracting LiLF] Box size, in degrees, outside of which to subtract
        the LiLF model from the data.

    - id: do_subtraction
      type: boolean?
      default: false
      doc: When set to true, the LiLF model will be subtracted from the DDF corrected data.
